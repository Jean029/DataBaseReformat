...
class Orders(dataBase.Model):
    ...
    tracking_num = dataBase.Column(
        dataBase.Integer, primary_key=False, autoincrement=True)
    order_date = dataBase.Column(dataBase.DateTime(
        timezone=True), default=func.now(), nullable=False)
    arrival_date = dataBase.Column(dataBase.DateTime(
        timezone=True), default=func.now(), nullable=False)
    ...
    status = dataBase.Column(dataBase.String(
        150), nullable=False, default='Pending')
