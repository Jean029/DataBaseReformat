...
def getorderid():
    return dataBase.session.execute(text('SELECT DISTINCT order_id FROM Order'))

# SQL para cambiar el total de la orden usando el id de la orden
def uptd_ordert():
    return dataBase.session.execute(text('UPDATE Order SET c_total = total_c WHERE id = order_id'))
                                         
# Escoger una orden especifico de un usuario
def sel_user_Ord():
    return dataBase.session.execute(text('SELECT * FROM Order where user_id = user_id')) #Añadir el user que se quieren ver las ordenes

# Escoger Ordenes dentro un rango de tiempo
def esp_range_orders():
    return dataBase.session.execute(text('SELECT * FROM Order WHERE order_date BETWEEN date1 AND date2')) 
# Anadir los ranges a los cuales se les quiere buscar tales ordenes

def get_trackingnum():
    return dataBase.session.execute(text('SELECT DISTINCT tracking_num FROM Order')) 
